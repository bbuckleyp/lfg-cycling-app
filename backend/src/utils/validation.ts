...
export const createEventSchema = z.object({
  eventType: z.enum(['ride', 'race'], { 
    errorMap: () => ({ message: 'Event type must be ride or race' })
  }),
  title: z.string().min(1, 'Title is required').max(255, 'Title too long'),
  description: z.string().max(1000, 'Description too long').optional(),
  startDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Date must be in YYYY-MM-DD format'),
  ...
  ridewithgpsRouteData: z.object({
    ridewithgpsRouteId: z.string().min(1, 'RideWithGPS route ID is required'),
    name: z.string().min(1, 'Route name is required'),
...
export const updateEventSchema = z.object({
  ...
  distanceMeters: z.number().int().min(0, 'Distance must be non-negative').nullable().optional(),
  elevationGainMeters: z.number().int().min(0, 'Elevation gain must be non-negative').nullable().optional(),
  isPublic: z.boolean().optional(),
  status: z.enum(['active', 'cancelled', 'completed']).optional(),
});